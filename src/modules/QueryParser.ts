...
class QueryParser {
	...
	tokenize(sql) {
		...
		const tokenRegex =
			/\s*(=>|!=|>=|<=|<>|[a-zA-Z_]\w*(?:\.[a-zA-Z_]\w*)?|@\w+|-?\d+(?:\.\d+)?|'(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*"|[(),=*.<>?])\s*/g;
		const tokens = [];
		let match;
	...
	parseCreate(tokens) {
		...
		if (tokens[0].toUpperCase() === "CREATE") {
			if (tokens[1].toUpperCase() !== "TABLE")
				throw new Error("Syntax: CREATE TABLE [name]");
			name = tokens[2];
		} else {
			if (tokens.length < 2)
				throw new Error("Syntax: LAHAN [nama_kebun]");
			name = tokens[1];
		}
	...
	parseShow(tokens) {
		...
		if (cmd === "LIHAT") {
			...
			if (sub === "INDEKS")
				return { type: "SHOW_INDEXES", table: tokens[2] || null };
		} else if (cmd === "SHOW") {
		...
		throw new Error(
			"Syntax: LIHAT LAHAN | SHOW TABLES | LIHAT INDEKS [table] | SHOW INDEXES",
		);
	}
	...
	parseInsert(tokens) {
		...
		if (tokens[0].toUpperCase() === "INSERT") {
			if (tokens[1].toUpperCase() !== "INTO")
				throw new Error("Syntax: INSERT INTO [table] ...");
			i = 2;
		} else {
			if (tokens[1].toUpperCase() !== "KE")
				throw new Error("Syntax: TANAM KE [kebun] ...");
			i = 2;
		}
		...
		if (tokens[i] === "(") {
			...
			while (tokens[i] !== ")") {
				...
				i++;
				if (i >= tokens.length)
					throw new Error("Unclosed parenthesis in columns");
			}
			i++;
		} else {
		...
		if (valueKeyword !== "BIBIT" && valueKeyword !== "VALUES")
			throw new Error("Expected BIBIT or VALUES");
		i++;
		...
		if (cols.length !== vals.length)
			throw new Error("Columns and Values count mismatch");
	...
	parseSelect(tokens) {
		...
		while (
			i < tokens.length &&
			!["DARI", "FROM"].includes(tokens[i].toUpperCase())
		) {
			if (tokens[i] !== ",") cols.push(tokens[i]);
			i++;
		}
		...
		while (i < tokens.length) {
			...
			if (token === "JOIN" || token === "GABUNG") {
				...
				if (
					tokens[i] &&
					["JOIN", "GABUNG"].includes(tokens[i].toUpperCase())
				) {
					joinType = "INNER";
					i++;
				}
			...
			if (
				i >= tokens.length ||
				!["ON", "PADA"].includes(tokens[i].toUpperCase())
			) {
				throw new Error(
					`Syntax: ${joinType} JOIN [table] ON [condition]`,
				);
			}
			i++; // Skip ON/PADA
		...
		if (
			i < tokens.length &&
			["DIMANA", "WHERE"].includes(tokens[i].toUpperCase())
		) {
			i++;
		...
		if (i < tokens.length) {
			...
			if (
				...
				(tokens[i - 1].toUpperCase() === "BY" ||
					tokens[i - 1].toUpperCase() === "BERDASARKAN")
			) {
				const key = tokens[i];
				i++;
				let dir = "asc";
		...
		if (
			i < tokens.length &&
			["LIMIT", "HANYA"].includes(tokens[i].toUpperCase())
		) {
			i++;
	...
	parseWhere(tokens, startIndex) {
		...
		while (i < tokens.length) {
			...
			if (["AND", "OR", "DAN", "ATAU"].includes(upper)) {
				// Map DAN->AND, ATAU->OR
				const op =
					upper === "DAN" ? "AND" : upper === "ATAU" ? "OR" : upper;
			...
			if (i < tokens.length - 1) {
				...
				if (op === "BETWEEN" || op === "ANTARA") {
					...
					let v2 = tokens[i + 4];
					// ... normalization ...
					if (v1 && (v1.startsWith("'") || v1.startsWith('"')))
						v1 = v1.slice(1, -1);
					else if (!isNaN(v1)) v1 = Number(v1);

					if (v2 && (v2.startsWith("'") || v2.startsWith('"')))
						v2 = v2.slice(1, -1);
					...
					if (connector !== "AND" && connector !== "DAN")
	...
	parseDelete(tokens) {
		...
		if (tokens[0].toUpperCase() === "DELETE") {
			if (tokens[1].toUpperCase() !== "FROM")
				throw new Error("Syntax: DELETE FROM [table] ...");
			table = tokens[2];
			i = 3;
		} else {
			if (tokens[1].toUpperCase() !== "DARI")
				throw new Error("Syntax: GUSUR DARI [kebun] ...");
			table = tokens[2];
			i = 3;
		}
	...
	parseUpdate(tokens) {
		...
		if (tokens[0].toUpperCase() === "UPDATE") {
			...
			if (tokens.length < 3)
				throw new Error("Syntax: PUPUK [kebun] DENGAN ...");
			table = tokens[1];
			if (tokens[2].toUpperCase() !== "DENGAN")
				throw new Error("Expected DENGAN");
			i = 3;
		}
		...
		while (
			...
			const key = tokens[i];
			if (tokens[i + 1] !== "=")
				throw new Error("Syntax: key=value in update list");
			let val = tokens[i + 2];
			if (val.startsWith("'") || val.startsWith('"'))
				val = val.slice(1, -1);
			else if (!isNaN(val)) val = Number(val);
	...
	parseCreateIndex(tokens) {
		...
		if (
			...
			if (
				tokens[i].toUpperCase() !== "ON" &&
				tokens[i + 1] &&
				tokens[i + 1].toUpperCase() === "ON"
			) {
				i++; // Skip name
			}

			if (tokens[i].toUpperCase() !== "ON")
				throw new Error("Syntax: CREATE INDEX ... ON [table] ...");
			i++;
			...
			if (tokens[i] !== "(")
				throw new Error("Syntax: ... ON [table] ( [field] )");
			i++;
			...
			if (tokens[i] !== ")")
				throw new Error("Unclosed parenthesis for index field");
		...
		// Tani Fallback
		if (tokens.length < 4)
			throw new Error("Syntax: INDEKS [table] PADA [field]");
		const table = tokens[1];
		if (tokens[2].toUpperCase() !== "PADA")
			throw new Error("Expected PADA");
	...
	parseAggregate(tokens) {
		...
		if (tokens[i] !== "(")
			throw new Error("Syntax: HITUNG FUNC(field) ...");
		i++;
		...
		if (
			!tokens[i] ||
			(tokens[i].toUpperCase() !== "DARI" &&
				tokens[i].toUpperCase() !== "FROM")
		) {
		...
		if (
			...
			while (
				i < tokens.length &&
				!["KELOMPOK", "GROUP", "DENGAN", "HAVING"].includes(
					tokens[i].toUpperCase(),
				)
			) {
				i++;
		...
		if (
			i < tokens.length &&
			["KELOMPOK", "GROUP"].includes(tokens[i].toUpperCase())
		) {
			// GROUP BY field or KELOMPOK field
			// Syntax: GROUP BY field
			if (
				tokens[i].toUpperCase() === "GROUP" &&
				tokens[i + 1].toUpperCase() === "BY"
			) {
				i += 2;
			} else {

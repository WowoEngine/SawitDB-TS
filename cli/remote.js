...
async function init() {
	try {
		...
		console.log(
			"  LAHAN [nama] | CREATE TABLE [name]         - Create table",
		);
		console.log(
			"  LIHAT LAHAN | SHOW TABLES                  - Show tables",
		);
		console.log(
			"  LIHAT INDEKS [table] | SHOW INDEXES [table]- Show indexes",
		);
		console.log("  MANIPULASI DATA (DML):");
		console.log(
			"  TANAM KE [table] ... BIBIT ...             - Insert Data",
		);
		console.log(
			"  PANEN [cols] DARI [table] DIMANA ...       - Select Data",
		);
		console.log("  ... URUTKAN BERDASARKAN [cal] [NAIK|TURUN] - Order By");
		console.log(
			"  ... HANYA [n] MULAI DARI [m]               - Limit & Offset",
		);
		console.log(
			"  ... KELOMPOK [col] DENGAN SYARAT ...       - Group By & Having",
		);
		console.log(
			"  PANEN UNIK [col] DARI ...                  - Select Distinct",
		);
		console.log(
			"  PUPUK [table] DENGAN ... DIMANA ...        - Update Data",
		);
		console.log(
			"  GUSUR DARI [table] DIMANA ...              - Delete Data",
		);
		console.log(
			"  HITUNG FUNC(field) DARI ...                - Aggregate (SUM, AVG, COUNT)",
		);
		console.log("");
		console.log("  RELASI (JOINS):");
		console.log(
			"  GABUNG [table] PADA ...                    - Inner Join",
		);
		console.log("  GABUNG KIRI [table] PADA ...               - Left Join");
		console.log(
			"  GABUNG KANAN [table] PADA ...              - Right Join",
		);
		console.log(
			"  GABUNG SILANG [table]                      - Cross Join",
		);
		console.log("");
		console.log("  LAIN-LAIN:");
		console.log(
			"  LAHAN [nama]                               - Create Table",
		);
		console.log(
			"  LIHAT LAHAN                                - Show Tables",
		);
		console.log(
			"  BAKAR LAHAN [table]                        - Drop Table",
		);
		console.log(
			"  INDEKS [table] PADA [field]                - Create Index",
		);
		console.log(
			"  JELASKAN PANEN ...                         - Explain Query",
		);
		console.log("");
...
function prompt() {
	rl.question(`${client.currentDatabase || "none"}> `, async (line) => {
		...
		if (
			upperCmd.startsWith("USE ") ||
			upperCmd.startsWith("MASUK WILAYAH ")
		) {
	...
	try {
		switch (command) {
			...
			case ".stats":
				...
				console.log(
					`  Memory: ${Math.round(stats.memoryUsage.heapUsed / 1024 / 1024)}MB / ${Math.round(stats.memoryUsage.heapTotal / 1024 / 1024)}MB`,
				);
				break;
